    ...
    match get_coordinates(city_name).await {
        Ok((lat, lng)) => match get_weather(lat as u32, lng as u32).await {
            Ok((weather_description, temperature)) => {
                println!("City: {}\nLatitude: {:.2}, Longitude: {:.2}\nCurrent Weather: {}\nTemperature: {:.2}°C", city_name, lat, lng, weather_description, temperature);
            }
...
async fn get_weather(lat: u32, lon: u32) -> Result<(String, f64), MyError> {
    ...
    let url = format!("https://api.openweathermap.org/data/3.0/onecall?lat={}&lon={}&exclude=minutely,hourly,daily,alerts&units=metric&appid={}", lat, lon, api_key);
